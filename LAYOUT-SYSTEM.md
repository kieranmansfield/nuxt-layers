...
2. **Unit.** `--unit: 1rlh`. `rlh` not `lh`: `lh` resolves against the
   _current_ element, so a heading with different line-height would
...
6. **Margins.** Whatever's left over, split 40/60 top/bottom so the block
   sits above optical centre — computed last because it _can't_ be chosen,
...
actual _used_ track list, implicit tracks included, even when the
...
- `grid-auto-columns: minmax(var(--measure-min), 1fr)` so _any_ forced
