...
<template>
  ...
  <div class="relative" style="aspect-ratio: 1; width: 100%">
    ...
    <div
      class="absolute rounded-full bg-(--ui-bg)"
    ...
    >
      ...
      <line
        :x1="50" :y1="50" :x2="basePos.x" :y2="basePos.y"
        stroke="white" stroke-width="0.8" stroke-opacity="0.7" stroke-dasharray="2 2"
      />
      <line
        v-for="(pos, i) in harmonyPositions"
        :key="`l${i}`"
        :x1="50" :y1="50" :x2="pos.x" :y2="pos.y"
        stroke="white" stroke-width="0.6" stroke-opacity="0.4" stroke-dasharray="2 2"
      />
      ...
      <g v-for="(pos, i) in harmonyPositions" :key="`d${i}`">
        ...
        <circle :cx="pos.x" :cy="pos.y" :r="DOT_R" :fill="pos.hex" stroke="white" stroke-width="1.5" />
      </g>

      <!-- Base dot (on top) -->
      <circle :cx="basePos.x" :cy="basePos.y" :r="DOT_R + 2.5" fill="rgba(255,255,255,0.3)" />
      <circle :cx="basePos.x" :cy="basePos.y" :r="DOT_R + 0.5" :fill="baseHex" stroke="white" stroke-width="2" />
    </svg>
  </div>
</template>
