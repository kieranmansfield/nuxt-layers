...
<template>
  <div class="min-h-screen bg-neutral-950 text-white flex flex-col">
    ...
    <div class="flex items-center gap-4 px-6 py-4 border-b border-white/8">
      <UButton to="/shader-pipeline" variant="ghost" icon="i-lucide-arrow-left" size="xs" color="neutral" />
      <div>
        <h1 class="font-semibold text-sm text-white">Shader Pipeline Backgrounds</h1>
        <p class="text-xs text-white/40">50+ background presets · Gradients, Mesh, Noise, Atmospheric, Abstract</p>
      </div>
    </div>
    ...
    <div class="flex flex-1 overflow-hidden">
      ...
      <div class="flex-1 flex items-center justify-center p-6">
        <div class="relative w-full max-w-xl aspect-video rounded-2xl overflow-hidden ring-1 ring-white/10 shadow-2xl">
          <ClientOnly>
              ...
              <div class="hidden pointer-events-none" aria-hidden="true">
                ...
                <template v-if="activeCategory === 'gradients'">
                  ...
                  <template v-else-if="activeGrad === 'radial'">
                    <PipelineRadialGradient
                      :order="0" :color-center="grad.colorCenter" :color-edge="grad.colorA"
                      :radius="grad.radius"
                    />
                  </template>
                  <template v-else-if="activeGrad === 'focal'">
                    <PipelineFocalGradient
                      :order="0" :color-a="grad.colorA" :color-b="grad.colorB"
                      :focal="[grad.focalX, grad.focalY]" :radius="grad.radius"
                    />
                  </template>
                  <template v-else-if="activeGrad === 'conic'">
                    <PipelineConicGradient
                      :order="0" :color-a="grad.colorA" :color-b="grad.colorB" :rotation="grad.rotation"
                    />
                  </template>
                ...
                <template v-else-if="activeCategory === 'mesh'" :key="activeMesh">
                  <template v-if="activeMesh === 'classic'">
                    <PipelineBilinearGradient
                      :order="0" :bottom-left="mesh.bl" :bottom-right="mesh.br"
                      :top-left="mesh.tl" :top-right="mesh.tr"
                    />
                  </template>
                  <template v-else-if="activeMesh === 'warm'">
                    <PipelineBilinearGradient
                      :order="0" bottom-left="#ff6600" bottom-right="#ffcc00"
                      top-left="#cc2200" top-right="#ff9944"
                    />
                  </template>
                  <template v-else-if="activeMesh === 'cool'">
                    <PipelineBilinearGradient
                      :order="0" bottom-left="#001166" bottom-right="#0044cc"
                      top-left="#220066" top-right="#4400aa"
                    />
                  </template>
                  <template v-else-if="activeMesh === 'pastel'">
                    <PipelineBilinearGradient
                      :order="0" bottom-left="#ffd6e0" bottom-right="#d6f0ff"
                      top-left="#e8d6ff" top-right="#d6ffe8"
                    />
                  </template>
                  ...
                  <template v-else-if="activeMesh === 'grainient'">
                    <PipelineUVNoiseRotate :order="0" :scale="mesh.warpScale" :speed="mesh.warpSpeed" />
                    <PipelineRotatedGradientBlend
                      :order="0" :color-a="mesh.bl" :color-b="mesh.tr"
                      :noise-scale="mesh.noiseScale" :speed="mesh.speed"
                    />
                  </template>
                  <template v-else-if="activeMesh === 'noisy'">
                    <PipelineNoisyGradient
                      :order="0" :color-a="mesh.bl" :color-b="mesh.tr"
                      :noise-scale="mesh.noiseScale" :noise-strength="mesh.noiseStrength" :speed="mesh.speed"
                    />
                  </template>
                  <template v-else-if="activeMesh === 'full-grainient'">
                    <PipelineNoisyGradientBlend
                      :order="0" :color-a="mesh.bl" :color-b="mesh.tr" :speed="mesh.speed"
                    />
                  </template>
                ...
                <template v-else-if="activeCategory === 'animated'" :key="activeAnim">
                  <template v-if="activeAnim === 'flowing'">
                    <PipelineNoisyGradientBlend
                      :order="0" :color-a="anim.colorA" :color-b="anim.colorB" :speed="0.2"
                    />
                  </template>
                  <template v-else-if="activeAnim === 'wave-colour'">
                    <PipelineLinearGradient :order="0" :color-a="anim.colorA" :color-b="anim.colorB" axis="y" />
                    <PipelineWaveBendLayer :order="1" :color="anim.waveColor" :opacity="anim.waveOpacity" :frequency="anim.waveFreq" :speed="anim.waveSpeed" />
                    <PipelineWaveBendLayer :order="2" :color="anim.colorA" :opacity="0.5" :frequency="anim.waveFreq * 1.5" :speed="anim.waveSpeed * 0.7" />
                  </template>
                  <template v-else-if="activeAnim === 'sine-warp'">
                    <PipelineUVSineWarpXY :order="0" :frequency="anim.warpFreq" :amplitude="anim.warpAmp" :speed="anim.warpSpeed" />
                    <PipelineRotatedGradientBlend :order="0" :color-a="anim.colorA" :color-b="anim.colorB" />
                  </template>
                  <template v-else-if="activeAnim === 'noisy-linear'">
                    <PipelineNoisyGradient
                      :order="0" :color-a="anim.colorA" :color-b="anim.colorB"
                      :noise-scale="anim.fbmScale" :speed="anim.fbmSpeed"
                    />
                  </template>
                  ...
                  <template v-else-if="activeAnim === 'shifting'">
                    <PipelineNoisyGradientBlend
                      :order="0" :color-a="anim.colorA" :color-b="anim.colorB" :speed="0.5"
                    />
                  </template>
                  <template v-else-if="activeAnim === 'fbm-flow'" :key="anim.fbmOctaves">
                    <PipelineFBMNoise
                      :order="0" :scale="anim.fbmScale" :speed="anim.fbmSpeed" :octaves="anim.fbmOctaves"
                      color-a="#000000" color-b="#ffffff"
                    />
                  ...
                  <template v-else-if="activeAnim === 'aurora-grad'">
                    ...
                    <PipelineAurora
                      :order="1" :color-a="anim.auroraColorA" :color-b="anim.auroraColorB"
                      :band-y="anim.auroraBandY" :intensity="anim.auroraIntensity"
                    />
                ...
                <!-- ══ NOISE BACKGROUNDS ═════════════════════════════ -->
                <template v-else-if="activeCategory === 'noise_bg'" :key="activeNoiseBg + noiseBg.octaves">
                  <template v-if="activeNoiseBg === 'fbm-classic'" :key="noiseBg.octaves">
                    <PipelineFBMNoise
                      :order="0" :scale="noiseBg.scale" :speed="noiseBg.speed" :octaves="noiseBg.octaves"
                      color-a="#000000" color-b="#ffffff"
                    />
                  ...
                  <template v-else-if="activeNoiseBg === 'domain-warp'">
                    <PipelineDomainWarpedNoise :order="0" :scale="noiseBg.scale" :speed="noiseBg.speed" />
                    <PipelineDuoTone :order="1" shadow-color="#1a0033" highlight-color="#ffcc00" />
                    <PipelineVignette :order="2" :intensity="noiseBg.vignetteIntensity" />
                  </template>
                  <template v-else-if="activeNoiseBg === 'voronoi'">
                    <PipelineVoronoiEdges :order="0" :scale="noiseBg.scale" :speed="noiseBg.speed" />
                  ...
                  <template v-else-if="activeNoiseBg === 'cellular'">
                    <PipelineCellularNoise :order="0" :scale="noiseBg.scale" :speed="noiseBg.speed" />
                    <PipelineMonochromeTint :order="1" />
                    <PipelineVignette :order="2" :intensity="noiseBg.vignetteIntensity" />
                  </template>
                  <template v-else-if="activeNoiseBg === 'simplex'">
                    <PipelineSimplexNoise :order="0" :scale="noiseBg.scale" :speed="noiseBg.speed" />
                    <PipelineBrightnessContrast
                      :order="1" :brightness="noiseBg.brightness" :contrast="noiseBg.contrast"
                    />
                    <PipelineVignette :order="2" :intensity="noiseBg.vignetteIntensity" />
                  </template>
                </template>
                ...
                <template v-else-if="activeCategory === 'atmospheric'" :key="activeAtmo">
                  <template v-if="activeAtmo === 'night-sky'">
                    ...
                    <PipelineAurora
                      :order="1" :color-a="atmo.auroraColorA" :color-b="atmo.auroraColorB"
                      :band-y="atmo.auroraBandY" :band-height="atmo.auroraBandHeight"
                      :intensity="atmo.auroraIntensity" :speed="atmo.auroraSpeed"
                    />
                    <PipelineStarfield :order="2" :density="atmo.starDensity" :brightness="atmo.starBrightness" />
                    <PipelineHaze :order="3" :color="atmo.hazeColor" :reach="atmo.hazeReach" :intensity="atmo.hazeIntensity" />
                  </template>
                  <template v-else-if="activeAtmo === 'day-sky'">
                    <PipelineSkyAtmosphere :order="0" :sun-direction="[0.3, atmo.sunDirectionY, 0.5]" />
                  </template>
                  ...
                  <template v-else-if="activeAtmo === 'deep-space'">
                    ...
                    <PipelineStarfield :order="1" :density="atmo.starDensity * 2" :brightness="atmo.starBrightness" :twinkle-speed="0.5" />
                ...
                <template v-else-if="activeCategory === 'abstract'" :key="activeAbstract">
                  <template v-if="activeAbstract === 'tunnel'">
                    <PipelineRaymarchTunnel
                      :order="0" :radius="abstract.tunnelRadius" :speed="abstract.tunnelSpeed"
                      :color-a="abstract.tunnelColorA" :color-b="abstract.tunnelColorB"
                    />
                  </template>
                  <template v-else-if="activeAbstract === 'complex'">
                    <PipelineComplexPlaneField
                      :order="0" :pole-angle="abstract.complexPoleAngle"
                      :pole-distance="abstract.complexPoleDistance" :speed="abstract.complexSpeed"
                    />
                    <PipelineCosinePalette :order="1" scalar-source="prev" />
                  ...
                  <template v-else-if="activeAbstract === 'marble'">
                    <PipelineMarble
                      :order="0" :color-a="abstract.marbleColorA" :color-b="abstract.marbleColorB"
                  ...
                  <template v-else-if="activeAbstract === 'wood'">
                    <PipelineWood :order="0" :ring-frequency="abstract.woodRingFrequency" :noise-strength="abstract.woodNoiseStrength" />
                  ...
                  <template v-else-if="activeAbstract === 'flame'">
                    <PipelineFlame
                      :order="0" :color-base="abstract.flameColorBase" :color-tip="abstract.flameColorTip"
                    />
                    <PipelineVignette :order="1" :intensity="abstract.vignetteIntensity" />
                  </template>
                  <template v-else-if="activeAbstract === 'water'">
                    <PipelineWater
                      :order="0" :color-deep="abstract.waterDeep" :color-shallow="abstract.waterShallow"
                      :frequency="abstract.waterFrequency" :speed="abstract.waterSpeed"
                    />
                    <PipelineVignette :order="1" :intensity="abstract.vignetteIntensity" />
                  </template>
                </template>
        ...
        <div class="flex-1 overflow-y-auto p-4 space-y-5">
          ...
          <div>
            ...
            <div v-else-if="activeCategory === 'mesh'" class="grid grid-cols-3 gap-1.5">
              <button
                v-for="p in meshPresets" :key="p.id"
                class="px-2 py-2 rounded-lg text-xs text-center transition-all leading-tight"
                :class="activeMesh === p.id
                  ? 'bg-violet-500/25 text-violet-200 ring-1 ring-violet-500/40'
                  : 'bg-white/5 text-white/50 hover:text-white/80 hover:bg-white/8'"
            ...
            <div v-else-if="activeCategory === 'animated'" class="grid grid-cols-3 gap-1.5">
              <button
                v-for="p in animatedPresets" :key="p.id"
                class="px-2 py-2 rounded-lg text-xs text-center transition-all leading-tight"
                :class="activeAnim === p.id
                  ? 'bg-violet-500/25 text-violet-200 ring-1 ring-violet-500/40'
                  : 'bg-white/5 text-white/50 hover:text-white/80 hover:bg-white/8'"
            ...
            <div v-else-if="activeCategory === 'noise_bg'" class="grid grid-cols-3 gap-1.5">
              <button
                v-for="p in noiseBgPresets" :key="p.id"
                class="px-2 py-2 rounded-lg text-xs text-center transition-all leading-tight"
                :class="activeNoiseBg === p.id
                  ? 'bg-violet-500/25 text-violet-200 ring-1 ring-violet-500/40'
                  : 'bg-white/5 text-white/50 hover:text-white/80 hover:bg-white/8'"
            ...
            <div v-else-if="activeCategory === 'atmospheric'" class="grid grid-cols-3 gap-1.5">
              <button
                v-for="p in atmosphericPresets" :key="p.id"
                class="px-2 py-2 rounded-lg text-xs text-center transition-all leading-tight"
                :class="activeAtmo === p.id
                  ? 'bg-violet-500/25 text-violet-200 ring-1 ring-violet-500/40'
                  : 'bg-white/5 text-white/50 hover:text-white/80 hover:bg-white/8'"
            ...
            <div v-else-if="activeCategory === 'abstract'" class="grid grid-cols-3 gap-1.5">
              <button
                v-for="p in abstractPresets" :key="p.id"
                class="px-2 py-2 rounded-lg text-xs text-center transition-all leading-tight"
                :class="activeAbstract === p.id
                  ? 'bg-violet-500/25 text-violet-200 ring-1 ring-violet-500/40'
                  : 'bg-white/5 text-white/50 hover:text-white/80 hover:bg-white/8'"
          ...
          <div class="space-y-4">
            ...
            <template v-if="activeCategory === 'gradients'">
              <template v-if="activeGrad === 'radial'">
                ...
                <DemoSliderRow label="Radius" :min="0.3" :max="2" :step="0.05" v-model="grad.radius" />
              </template>
              <template v-else-if="activeGrad === 'focal'">
                <DemoColorRow label="Color A" v-model="grad.colorA" />
                <DemoColorRow label="Color B" v-model="grad.colorB" />
                <DemoSliderRow label="Focal X" :min="0" :max="1" :step="0.01" v-model="grad.focalX" />
                <DemoSliderRow label="Focal Y" :min="0" :max="1" :step="0.01" v-model="grad.focalY" />
                <DemoSliderRow label="Radius" :min="0.3" :max="2" :step="0.05" v-model="grad.radius" />
              </template>
              <template v-else-if="activeGrad === 'conic'">
                <DemoColorRow label="Color A" v-model="grad.colorA" />
                <DemoColorRow label="Color B" v-model="grad.colorB" />
                <DemoSliderRow label="Rotation" :min="0" :max="360" :step="5" v-model="grad.rotation" />
              </template>
              <template v-else-if="activeGrad !== 'sunrise' && activeGrad !== 'pastel' && activeGrad !== 'sunset' && activeGrad !== 'ocean'">
                <DemoColorRow label="Color A" v-model="grad.colorA" />
                <DemoColorRow label="Color B" v-model="grad.colorB" />
              </template>
              <template v-if="activeGrad === 'ocean' || activeGrad === 'sunset'">
                <DemoSliderRow label="Vignette" :min="0" :max="1" :step="0.01" v-model="grad.vignetteIntensity" />
              </template>
            </template>

            <!-- ── Mesh gradient controls ── -->
            <template v-else-if="activeCategory === 'mesh'">
              <template v-if="activeMesh === 'classic' || activeMesh === 'organic' || activeMesh === 'warped'">
              ...
              <template v-else-if="activeMesh === 'grainient' || activeMesh === 'noisy' || activeMesh === 'full-grainient' || activeMesh === 'multi-focal'">
              ...
              <template v-if="activeMesh === 'organic' || activeMesh === 'warped'">
                ...
                <DemoSliderRow label="Warp Speed" :min="0" :max="0.5" :step="0.01" v-model="mesh.warpSpeed" />
              </template>
              <template v-if="activeMesh === 'noisy' || activeMesh === 'warped' || activeMesh === 'grainient'">
                <div class="h-px bg-white/5" />
                <DemoSliderRow label="Noise Scale" :min="0.5" :max="5" :step="0.25" v-model="mesh.noiseScale" />
                <DemoSliderRow label="Speed" :min="0" :max="0.5" :step="0.01" v-model="mesh.speed" />
              </template>
            ...
            <template v-else-if="activeCategory === 'animated'">
              ...
              <template v-else-if="activeAnim === 'wave-colour'">
                ...
                <DemoSliderRow label="Wave Opacity" :min="0" :max="1" :step="0.05" v-model="anim.waveOpacity" />
                <DemoSliderRow label="Frequency" :min="1" :max="8" :step="0.5" v-model="anim.waveFreq" />
                <DemoSliderRow label="Speed" :min="0" :max="2" :step="0.1" v-model="anim.waveSpeed" />
              </template>
              <template v-else-if="activeAnim === 'sine-warp'">
                <DemoColorRow label="Color A" v-model="anim.colorA" />
                <DemoColorRow label="Color B" v-model="anim.colorB" />
                <DemoSliderRow label="Frequency" :min="1" :max="15" :step="0.5" v-model="anim.warpFreq" />
                <DemoSliderRow label="Amplitude" :min="5" :max="80" :step="1" v-model="anim.warpAmp" />
                <DemoSliderRow label="Speed" :min="0" :max="5" :step="0.1" v-model="anim.warpSpeed" />
              </template>
              <template v-else-if="activeAnim === 'fbm-flow'">
                <DemoSliderRow label="Scale" :min="1" :max="10" :step="0.5" v-model="anim.fbmScale" />
                <DemoSliderRow label="Speed" :min="0" :max="0.5" :step="0.01" v-model="anim.fbmSpeed" />
                <DemoSliderRow label="Octaves" :min="1" :max="8" :step="1" v-model="anim.fbmOctaves" />
              </template>
              <template v-else-if="activeAnim === 'aurora-grad'">
                <DemoColorRow label="Aurora A" v-model="anim.auroraColorA" />
                <DemoColorRow label="Aurora B" v-model="anim.auroraColorB" />
                <DemoSliderRow label="Band Y" :min="0.2" :max="0.9" :step="0.01" v-model="anim.auroraBandY" />
                <DemoSliderRow label="Intensity" :min="0" :max="1" :step="0.05" v-model="anim.auroraIntensity" />
              </template>
            ...
            <template v-else-if="activeCategory === 'noise_bg'">
              ...
              <DemoSliderRow label="Speed" :min="0" :max="0.5" :step="0.01" v-model="noiseBg.speed" />
              <DemoSliderRow
                v-if="activeNoiseBg === 'fbm-classic'" label="Octaves"
                :min="1" :max="8" :step="1" v-model="noiseBg.octaves"
              />
              <template v-if="activeNoiseBg === 'curl-noise' || activeNoiseBg === 'billow'">
                <DemoSliderRow label="Hue Shift" :min="0" :max="1" :step="0.01" v-model="noiseBg.hue" />
              </template>
              <template v-if="activeNoiseBg === 'simplex'">
                <DemoSliderRow label="Brightness" :min="-0.5" :max="0.5" :step="0.01" v-model="noiseBg.brightness" />
                <DemoSliderRow label="Contrast" :min="0.5" :max="2.5" :step="0.05" v-model="noiseBg.contrast" />
              </template>
              <DemoSliderRow label="Vignette" :min="0" :max="1" :step="0.01" v-model="noiseBg.vignetteIntensity" />
            </template>
            ...
            <template v-else-if="activeCategory === 'atmospheric'">
              <template v-if="activeAtmo === 'night-sky' || activeAtmo === 'aurora-only'">
                ...
                <DemoSliderRow label="Band Y" :min="0.2" :max="0.9" :step="0.01" v-model="atmo.auroraBandY" />
                <DemoSliderRow label="Band Height" :min="0.05" :max="0.6" :step="0.01" v-model="atmo.auroraBandHeight" />
                <DemoSliderRow label="Intensity" :min="0" :max="1" :step="0.01" v-model="atmo.auroraIntensity" />
                <DemoSliderRow label="Speed" :min="0" :max="1" :step="0.05" v-model="atmo.auroraSpeed" />
              </template>
              <template v-if="activeAtmo === 'night-sky'">
                <div class="h-px bg-white/5" />
                <DemoSliderRow label="Star Density" :min="20" :max="200" :step="5" v-model="atmo.starDensity" />
                <DemoSliderRow label="Star Brightness" :min="0.1" :max="2" :step="0.05" v-model="atmo.starBrightness" />
                <DemoSliderRow label="Haze Reach" :min="0" :max="0.7" :step="0.01" v-model="atmo.hazeReach" />
                <DemoSliderRow label="Haze Intensity" :min="0" :max="0.6" :step="0.01" v-model="atmo.hazeIntensity" />
              </template>
              <template v-else-if="activeAtmo === 'day-sky'">
                <DemoSliderRow label="Sun Height" :min="0" :max="1" :step="0.01" v-model="atmo.sunDirectionY" />
              </template>
              <template v-else-if="activeAtmo === 'day-night'">
                <DemoSliderRow label="Cycle Speed" :min="0" :max="1" :step="0.01" v-model="atmo.dayNightSpeed" />
              </template>
              <template v-else-if="activeAtmo === 'deep-space'">
                <DemoSliderRow label="Star Density" :min="50" :max="300" :step="10" v-model="atmo.starDensity" />
                <DemoSliderRow label="Star Brightness" :min="0.5" :max="3" :step="0.1" v-model="atmo.starBrightness" />
              </template>
            </template>
            ...
            <template v-else-if="activeCategory === 'abstract'">
              ...
              <template v-else-if="activeAbstract === 'marble'">
                ...
                <DemoSliderRow label="Frequency" :min="1" :max="15" :step="0.5" v-model="abstract.marbleFrequency" />
              </template>
              <template v-else-if="activeAbstract === 'wood'">
                <DemoSliderRow label="Ring Frequency" :min="4" :max="30" :step="1" v-model="abstract.woodRingFrequency" />
                <DemoSliderRow label="Noise Strength" :min="0" :max="1" :step="0.05" v-model="abstract.woodNoiseStrength" />
              </template>
              <template v-else-if="activeAbstract === 'flame'">
              ...
              <template v-else-if="activeAbstract === 'water'">
                ...
                <DemoSliderRow label="Frequency" :min="2" :max="20" :step="0.5" v-model="abstract.waterFrequency" />
                <DemoSliderRow label="Speed" :min="0" :max="3" :step="0.1" v-model="abstract.waterSpeed" />
              </template>
              <template v-if="activeAbstract !== 'tunnel' && activeAbstract !== 'complex'">
                <DemoSliderRow label="Vignette" :min="0" :max="1" :step="0.01" v-model="abstract.vignetteIntensity" />
              </template>
            </template>
