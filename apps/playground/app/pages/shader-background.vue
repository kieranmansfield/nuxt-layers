<script setup lang="ts">
  ...
  const grain2Key = computed(
    () => `g2-${grain2State.type}-${grain2State.blendMode}-${grain2State.style}`
  )
  ...
  function getGrainBlock(): string {
    return grainBlockStr(grainState, 98)
  }
  function getGrainBlock2(): string {
    return grainBlockStr(grain2State, 99)
  }
                ...
                <div class="hidden pointer-events-none" aria-hidden="true">
                  ...
                  <template v-if="grain2Enabled" :key="grain2Key">
                    <PipelineGrain
                      ...
                      :order="99"
                      :intensity="grain2State.intensity"
                      :opacity="grain2State.opacity"
                      :size="grain2State.size"
                      :style="grain2State.style"
                      :animated="grain2State.animated"
                      :fps="grain2State.fps"
                    ...
                    <PipelineRisographGrain
                      ...
                      :order="99"
                      :scale="grain2State.scale"
                      :strength="grain2State.strength"
                      :opacity="grain2State.opacity"
                      :fps="grain2State.fps"
                      :blend-mode="grain2State.blendMode"
                    />
              ...
              <div class="hidden pointer-events-none" aria-hidden="true">
                ...
                <template v-else-if="activeCategory === 'mesh'" :key="activeMesh">
                  ...
                  <template v-else-if="activeMesh === 'organic'">
                    ...
                    <PipelineUVSineWarpXY
                      ...
                      :order="0"
                      :frequency="mesh.warpScale * 2"
                      :amplitude="40"
                      :speed="mesh.warpSpeed * 2"
                    />
                    <PipelineUVTwirl
                      v-else-if="mesh.organicType === 'twirl'"
                      :order="0"
                      :strength="mesh.warpStrength * 2"
                    />
                    <PipelineBilinearGradient
                      :order="0"
                      :bottom-left="mesh.bl"
                  ...
                  <template v-else-if="activeMesh === 'grainient'">
                    ...
                    <PipelineRotatedGradientBlend
                      ...
                      :speed="mesh.grainientSpeed"
                      :edge0="mesh.grainientEdge0"
                      :edge1="mesh.grainientEdge1"
                    />
                  ...
                  <template v-else-if="activeMesh === 'warped'">
                    ...
                    <PipelineUVSineWarpXY
                      :order="0"
                      :frequency="mesh.warpScale * 1.5"
                      :amplitude="35"
                      :speed="mesh.warpSpeed * 1.5"
                    />
                    <PipelineBilinearGradient
                      :order="0"
                      :bottom-left="mesh.bl"
                ...
                <template v-else-if="activeCategory === 'abstract'" :key="activeAbstract">
                  ...
                  <template v-else-if="activeAbstract === 'chromatic-flow'">
                    <PipelineCurlNoise
                      :order="0"
                      :scale="abstract.flowScale"
                      :speed="abstract.flowSpeed"
                    />
                  ...
                  <template v-else-if="activeAbstract === 'cell-art'">
                    <PipelineVoronoiEdges
                      :order="0"
                      :scale="abstract.cellScale"
                      :speed="abstract.cellSpeed"
                    />
                    <PipelineDuoTone
                      :order="1"
                      :shadow-color="abstract.cellColorA"
                      :highlight-color="abstract.cellColorB"
                    />
                  ...
                  <template v-else-if="activeAbstract === 'holographic'">
                    <PipelineSimplexNoise
                      :order="0"
                      :scale="abstract.holoScale"
                      :speed="abstract.holoSpeed"
                    />
        ...
        <div class="flex-1 overflow-y-auto p-4 space-y-5">
          ...
          <div class="space-y-4">
            ...
            <template v-if="activeCategory === 'gradients'">
              ...
              <template v-else-if="!['sunrise', 'pastel', 'sunset', 'ocean'].includes(activeGrad)">
                <DemoColorRow v-model="grad.colorA" label="Color A" />
                <DemoColorRow v-model="grad.colorB" label="Color B" />
                ...
                <template v-if="['linear-h', 'linear-v'].includes(activeGrad)">
                  ...
                  <DemoColorRow v-if="grad.numColors >= 3" v-model="grad.colorC" label="Color C" />
                  <DemoColorRow v-if="grad.numColors >= 4" v-model="grad.colorD" label="Color D" />
                </template>
              </template>
            ...
            <template v-else-if="activeCategory === 'mesh'">
              ...
              <template v-if="activeMesh === 'grainient'">
                ...
                />
                <DemoSliderRow
                  v-model="mesh.grainientWarpSpeed"
                  label="Warp Speed"
                  :min="0.005"
                  :max="0.1"
                  :step="0.005"
                />
                <DemoSliderRow
                  v-model="mesh.grainientEdge0"
                  label="Color A Weight"
                  :min="0"
                  :max="0.5"
                  :step="0.01"
                />
                <DemoSliderRow
                  v-model="mesh.grainientEdge1"
                  label="Blend Width"
                  :min="0.1"
                  :max="0.9"
                  :step="0.02"
            ...
            <template v-else-if="activeCategory === 'abstract'">
              ...
              <template v-else-if="activeAbstract === 'neon-rings'">
                <DemoColorRow v-model="abstract.ringsColorA" label="Glow Color" />
                <DemoSliderRow
                  v-model="abstract.ringsFrequency"
                  label="Frequency"
                  :min="5"
                  :max="40"
                  :step="1"
              ...
              <template v-else-if="activeAbstract === 'cell-art'">
                <DemoColorRow v-model="abstract.cellColorA" label="Shadow" />
                <DemoColorRow v-model="abstract.cellColorB" label="Highlight" />
                <DemoSliderRow
                  v-model="abstract.cellScale"
                  label="Scale"
                  :min="1"
                  :max="10"
                  :step="0.5"
            ...
            <!-- ── Theme controls ── -->
            <template v-if="activeCategory === 'theme'">
              <DemoColorRow v-model="theme.color1" label="Color 1" />
              <DemoColorRow v-model="theme.color2" label="Color 2" />
              <DemoColorRow v-model="theme.color3" label="Color 3" />
              <DemoColorRow v-model="theme.color4" label="Color 4" />
              <DemoSliderRow v-model="theme.speed" label="Speed" :min="0.1" :max="3" :step="0.1" />
              <DemoSliderRow
                v-model="theme.intensity"
                label="Intensity"
                :min="0.1"
                :max="2"
                :step="0.1"
