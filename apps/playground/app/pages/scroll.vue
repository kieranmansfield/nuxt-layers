<script setup lang="ts">
  ...
  const directives = [
    { name: 'v-gsap.from', description: 'Animate from values on mount', example: 'v-gsap.from="{ y: 100, opacity: 0, duration: 1 }"' },
    { name: 'v-gsap.to', description: 'Animate to values on mount', example: 'v-gsap.to="{ x: 200, duration: 0.5 }"' },
    { name: 'v-gsap.whenVisible', description: 'Trigger when element enters viewport', example: 'v-gsap.whenVisible.from="{ opacity: 0, y: 30 }"' },
    { name: 'v-gsap.once', description: 'Only trigger once (combine with whenVisible)', example: 'v-gsap.whenVisible.once.from="{ ... }"' },
    { name: 'v-gsap.stagger', description: 'Stagger children with ScrollTrigger', example: 'v-gsap.whenVisible.stagger.from="{ ... }"' },
    { name: 'v-gsap.parallax', description: 'Declarative parallax speed modifier', example: 'v-gsap.parallax.slower-5' },
  ]

  const components = [
    { name: 'MotionScrollProgress', description: 'Fixed scroll progress bar at the top of the page', icon: 'i-lucide-bar-chart-2' },
    { name: 'MotionScrollStats', description: 'Live velocity and progress debug overlay', icon: 'i-lucide-activity' },
    { name: 'MotionParallax', description: 'Wraps an element to apply declarative parallax speed', icon: 'i-lucide-layers-3' },
    { name: 'MotionScrollLink', description: 'Button/link that scrolls to a target element', icon: 'i-lucide-anchor' },
    { name: 'MotionScrollScene', description: 'Provides SCROLL_SCENE_KEY context for step-based scrollytelling', icon: 'i-lucide-film' },
    { name: 'MotionScrollStep', description: 'A step within a ScrollScene — activates based on scroll position', icon: 'i-lucide-list-ordered' },
    { name: 'MotionPinnedSection', description: 'Pins a section and staggers children as you scroll through it', icon: 'i-lucide-pin' },
    { name: 'MotionHorizontalScroll', description: 'Converts vertical scroll into horizontal panel navigation', icon: 'i-lucide-arrow-right-left' },
  ]
...
<template>
  <LayoutPage title="Scroll Layer Demo" description="GSAP + Locomotive Scroll base layer">
    <div class="bg-gray-950 min-h-screen">
      <!-- Scroll progress bar -->
      <MotionScrollProgress class="fixed top-0 left-0 right-0 z-50" :show-percentage="false" :height="3" />
          ...
          <div class="space-y-8 py-8">
              ...
              <UCard>
                ...
                <div class="space-y-3">
                  ...
                  >
                    <code
                      class="text-sm font-mono text-primary shrink-0 w-48"
                    >{{ dir.name }}</code>
