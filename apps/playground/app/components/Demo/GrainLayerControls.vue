<script setup lang="ts">
  ...
  const STYLES = [
    ...
    { id: 'coarse', label: 'Grit' },
  ] as const
  const BLENDS = [
    { id: 'add', label: 'Add' },
    { id: 'sub', label: 'Sub' },
    { id: 'screen', label: 'Screen' },
    { id: 'overlay', label: 'Ovly' },
  ...
  <div class="space-y-3">
    ...
    <template v-if="enabled">
      ...
      <DemoSliderRow
        ...
        :step="0.005"
      />
      <DemoSliderRow v-model="state.opacity" label="Opacity" :min="0" :max="1" :step="0.01" />
      <DemoSliderRow v-model="state.size" label="Size" :min="0.25" :max="8" :step="0.25" />
      <DemoSliderRow v-model="state.fps" label="FPS" :min="4" :max="60" :step="1" />
      <template
        v-if="state.type === 'riso' && state.scale !== undefined && state.strength !== undefined"
      >
        <DemoSliderRow v-model="state.scale" label="Scale" :min="0.1" :max="2" :step="0.05" />
        <DemoSliderRow
