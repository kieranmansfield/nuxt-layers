...
Currently `core.css` has no `@layer` order statement — the priority of `base` vs `components` vs `utilities` is established implicitly by Tailwind's own `@import 'tailwindcss'` internals, and where *your own* `base.css`/`layout.css` land relative to `@nuxt/ui`'s rules depends entirely on import order (today: `@nuxt/ui` imported before `layout.css`/`base.css`, so on an equal-specificity conflict your own rules currently win — but only because of source order, which breaks silently if anyone reorders the imports later).
