...
The grid is a single CSS `display: grid` on `<main>` (the `.grid-root` class). Everything else — sections, items — participates via CSS `subgrid`, meaning they inherit the parent's column and row lines rather than defining their own. No JS is involved in the grid itself.

```
<main class="grid-root">          ← defines the grid (6/12/18 cols, row height)
...
`LayoutMain` renders the `<main class="grid-root">`. `LayoutSection` renders `<section class="basesection">`. `LayoutGridItem` handles grid positioning via inline style or presets.
...
> `LayoutPage` is a fragment component — it intentionally provides no wrapper element. The `grid-root` grid lives on `LayoutMain` in the layout file, not in `LayoutPage` itself. Do not add a second `LayoutMain` inside `LayoutPage`.
...
Renders `<main class="grid-root">`. When `mode === 'disabled'` it renders a plain `<main>` with no grid classes.
...
Full-viewport subgrid section. Spans 12 rows (= 100vh). Inherits the parent `.grid-root` grid columns via `subgrid`.
...
**`.grid-root`** — Root grid container on `<main>`.
...
**Use `overflow-x: clip` instead.** `clip` cuts off overflow without creating a scroll container and is not subject to the forced `overflow-y` conversion. This is why `.grid-root`, `html`, and `body` all use `overflow-x: clip` in this layer.
...
`LayoutPage` renders no element of its own. The `grid-root` grid lives on `LayoutMain` inside a layout file. If you use `<LayoutPage>` on a page that uses the `default` layout (which doesn't include `LayoutMain`), the grid CSS never applies.
